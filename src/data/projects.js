export const projects = [
  {
    id: 2,
    title: "-----",
    description: "------",
    image: "----",
    techStack: ["-----", "-----", "-----", "-----"],
    githubUrl: "#",
    liveUrl: "#",
    status: "-----",
    featured: true
  },
   {
    id: 1,
    title: "TDE Tool - Tanishq`s Decoder & Encoder",
    description: "A fast, zero-dependency CLI tool for Base64 encoding and decoding. Built with pure Python — works on Windows, Linux, and macOS.",
    image: "https://res.cloudinary.com/dx1zgcm3j/image/upload/v1781026818/TDE_2_pmdhpj.png",
    techStack: ["Python", "Cli", "Base64", "Cross-Platfrom", "Devtools", "Encoder", "Decoder", "CyberSecurity", "Command-Line-Tool", "Url-Safe"],
    githubUrl: "https://github.com/tanishqzope/TDE-Tool",
    liveUrl: "#",
    status: "Live",
    featured: true
  },
];
